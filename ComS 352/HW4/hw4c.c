...
/* create the pipe */ if (pipe(fd) == -1) { fprintf(stderr,"Pipe failed");
 return 1;
...
} else { /* child process */ /* close the unused end of the pipe */ close(fd[WRITE END]);

/* read from the pipe */ read(fd[READ END], read msg, BUFFER SIZE);
